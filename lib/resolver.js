...
function Resolver(root, options) {
  ...
  if (!(this.remote = options.remote || options.remotes)) {
    this.remote = remotes(this.constructor.remotes, {
      out: this.out,
    });
  ...
  // note that we need this regardless if we actually install stuff
  // because we may need it when resolving dependencies with locals
...
/**
 * Resolve a component's local paths.
 *
 * @param {Object} branch
 * @api private
 */

Resolver.prototype.resolvePaths = function (branch) {
  // convert the paths to absolute paths
  branch.paths = (branch.node.paths || []).map(function (dir) {
