...
Resolver.prototype.tree = function* () {
  ...
  var ch = this.channel
  yield* ch.locals.flush(false)
  yield* ch.dependencies.flush(false)
  ...
  this.semver()
  yield* ch.semver.flush(false)
  yield* ch.installs.flush(false)
  return tree
}
...
Resolver.prototype.resolve = function (branch) {
  ...
  if (this.deps) {
    ...
    if (component.development)
      this.resolveDependencies(branch, component.development.dependencies)
  }

  // resolve locals in a different channel
  this.resolveLocals(branch, component.locals)
  if (component.development)
    this.resolveLocals(branch, component.development.locals)
}
