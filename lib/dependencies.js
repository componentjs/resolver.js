...
Resolver.prototype.branchDependency = function* (name, ref, parent) {
  ...
  var installed = false;
  if (branch.node.locals) {
    // if the component has locals,
    // then we'll just download it locally.
    // not going to lookup locals using HTTP calls!
    // we have to download the entire component.
    installed = true;
    yield* this._download(branch.resolvedRemotes, branch.name, branch.ref, true);
  }
  ...
  if (!installed && this.install) this.download(branch);
  return branch
}

/**
 * Download a component in a separate coroutine.
 *
 * @param {Object} branch
 * @api private
 */
