...
Resolver.prototype.branchLocal = function* (root, component, parent) {
  ...
  // canonical names for each component
  // locals are prefixed with ./ if local to this app,
  // and prefixed with the dependency's <user>-<repo>@<reference>/
  // if local to a remote dependency.
  branch.canonical = relative(this.root, root) || branch.name;
  // no ../ paths
  if (!branch.canonical.indexOf('../')) throw new Error('all paths must be within the build context. use symlinks insteads.')
  branch.context = this.getLocalContext(branch);
  branch.canonical = (branch.context
    ? branch.context.canonical
    : '.') + '/' + branch.canonical;
...
/**
 * Get the first dependency of a local.
 * This is for remote dependencies with their own
 * locals and paths. This will allow us to write the locals
 * as something like <user>-<repo>@<reference>/client/<local>
 *
 * @param {Object} branch
...
Resolver.prototype.getLocalContext = function (branch) {
  while (branch = branch.parent) {
    if (branch.type === 'dependency') return branch;
  }
}
